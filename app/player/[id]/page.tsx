...
export default function PlayerPage() {
  ...
  return <main className="player-detail-page"><header className="topbar"><Link className="brand" href="/"><span className="brand-glyph">P</span><span>PROSPI<br/><b>PLAYER DATA</b></span></Link><Link className="back-link" href="/">← 선수 목록</Link></header>
    <section className="detail-page">
      <div className="detail-art"><OriginalCardEffect group={card.group}/><span className="detail-series">{card.year} · {card.playerType === "pitcher" ? "투수" : "타자"} · {card.variant}</span><img className="detail-player-image" src={imageUrl(card)} alt={`${card.name} 대형 카드 원본`}/><span className="effect-id-label"><i/> EFFECT {effectIdForGroup(card.group)} · LIVE</span><span className="image-size">CL · EXTRACTED PNG</span></div>
      <div className="detail-info"><p className="eyebrow">PLAYER DETAIL</p><p className="reading">{card.roman || `IMAGE ${card.id}`}</p><h1>{card.name}</h1><div className="detail-tags"><span>{card.year}</span><span>{card.playerType === "pitcher" ? "투수 카드" : "타자 카드"}</span><span>VAR {card.variant}</span></div>
        ...
        <dl><div><dt>PLAYER ID</dt><dd>{card.playerId || "2015 구형 이미지 ID"}</dd></div><div><dt>IMAGE ID</dt><dd>{card.id}</dd></div><div><dt>EFFECT ID</dt><dd>ANSS_EF_{effectIdForGroup(card.group)}_L.CHK</dd></div><div><dt>THUMBNAIL</dt><dd>{card.file}</dd></div><div><dt>LARGE PHOTO</dt><dd>{card.largeFile}</dd></div></dl><div className="proof"><span>{card.verified ? "✓" : "△"}</span><p><strong>{card.verified ? "앱 매니페스트 검증 완료" : "CDN 갱신 파일"}</strong><br/>선수 원본 이미지 위에 해당 연도의 대표 S등급 원본 효과 레이어를 재생합니다.</p></div>
      </div>
    </section>
  </main>;
}
