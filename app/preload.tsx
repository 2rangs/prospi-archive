 ...
 */
const DEFERRED_STEPS: (() => Promise<unknown>)[] = [
  loadRefStats,
  loadSkills,
  loadKnown,
  loadEffectPool,
...
export function Preload({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let alive = true;
    // 첫 화면이 실제로 필요한 선수 원장만 기다린다. 표기값·스킬·이펙트
    // 색인은 화면이 열린 뒤 각 훅의 같은 모듈 캐시에 병렬 예열한다.
    void loadCards().catch(() => null).then(() => {
      if (!alive) return;
      setReady(true);
      for (const step of DEFERRED_STEPS) void step().catch(() => null);
    });
  ...
  return <div className="preload" role="status" aria-live="polite">
    ...
    <p className="preload-pct">LOADING</p>
  </div>;
}
