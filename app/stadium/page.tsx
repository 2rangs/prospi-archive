...
export default function VRoad() {
  ...
  useEffect(() => {
    ...
    const trail = new THREE.Line(new THREE.BufferGeometry(), new THREE.LineBasicMaterial({ color: 0xffe27a, transparent: true, opacity: 0.8 }));
    scene.add(trail);
    ...
    function setTrail(pts: THREE.Vector3[]) { trail.geometry.dispose(); trail.geometry = new THREE.BufferGeometry().setFromPoints(pts.length >= 2 ? pts : []); }
