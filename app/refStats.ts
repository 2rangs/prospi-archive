...
export function loadRefStats(): Promise<Record<string, RefStats> | null> {
  ...
  return new Promise(resolve => {
    ...
    if (waiters.length === 1) {
      void fetchGzipJson<Record<string, RefStats>>("/data/ref-stats.json.gz?v=20260831")
        .catch(() => null)
...
export function useRefStats(): Record<string, RefStats> | null {
  ...
  useEffect(() => {
    ...
    waiters.push(setV);
    if (waiters.length === 1) {
      fetchGzipJson<Record<string, RefStats>>("/data/ref-stats.json.gz?v=20260831")
        .catch(() => null)
        .then(j => { cache = j; waiters.splice(0).forEach(w => w(j)); });
    }
  }, []);
...
export function useRefStatsForId(id: string | undefined): RefStats | null {
  ...
  useEffect(() => {
    ...
    let alive = true;
    fetchGzipJson<Record<string, RefStats>>(`/data/ref-shards/${id.slice(0, 2)}.json.gz?v=20260831`)
